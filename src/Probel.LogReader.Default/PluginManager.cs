...
using Probel.LogReader.Core.Entitites;
...
namespace Probel.LogReader.Default
{
    public class PluginManager : IPluginManager, IPluginInfoManager
    {
        #region Fields

        private const string _pluginRepository = @"%appdata%\probel\logreader\plugins\";
        ...
        public IEnumerable<PluginInfo> GetPluginInfo()
        {
            ...
            var result = (from m in _metadataList ?? new List<IPluginMetadata>()
                          select new PluginInfo(m.PluginId, m.Name));
            return result;
        }
